...
class AVL{
    ...
    private:
        ...
        bool removeUtil(node<T> * & head, T x){
            ...
            if(x == head->key){
                if(head->left == nullptr || head->right == nullptr){  //被删节点是叶节点或只有一个儿子
            ...
            if(x < head->key){  //在左子树上删除
                if(removeUtil(x, head->left)) return true;    //删除后左子树没有变矮
                return adjust(head, 0);
            } else {    //在右子树上删除
                if(removeUtil(x, head->right)) return true;   //删除后右子树没有变矮
                return adjust(head, 1);
            }
        }
